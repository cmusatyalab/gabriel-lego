...
#   Ported to Python3 by: Manuel Olguín Muñoz <molguin@kth.se>
#
...
September 2019: Ported to Python3 by M. Olguin Munoz
'''
...
def get_mask(img, rtn_type="mask", th=0):
    '''
    Given a color or black-white image, return the mask where the pixels are
    non-zero.
    '''
    img_shape = img.shape
...
def get_edge_point(mask, direction):
    ...
    If there are multiple points at the end of the direction, returns one of
    them.
    Returns None if @mask is empty.
    @direction is a vector. For example, if direction == (1, -1),
    then returns the most upper right point.
...
def make_convex(mask, use_approxPolyDp=True, use_convexHull=True,
                app_ratio=0.01, combine_cnts=False):
    ...
    if combine_cnts:
        ...
        if use_approxPolyDp:
            cnt_combine = cv2.approxPolyDP(cnt_combine,
                                           app_ratio * cv2.arcLength(
                                               cnt_combine, True), True)
        if use_convexHull:
        ...
        for cnt_idx, cnt in enumerate(contours):
            ...
            if use_approxPolyDp:
                cnt = cv2.approxPolyDP(cnt,
                                       app_ratio * cv2.arcLength(cnt, True),
                                       True)
            if use_convexHull:
...
def find_largest_CC(mask, min_convex_rate=0, min_area=0, ref_p=None,
                    max_dist_ref_p=0):
    '''
    Find largest connected component in a mask image, with minimum
    @min_convex_rate and @min_area.
    Can also set a reference point @ref_p so that the center of the found
    connected component is at maximum @max_dist_ref_p to the reference point.
    Return a mask with only the largest connected component drawn, as well as
    the max contour area.
    Returns (None, -1) if nothing good found.
    '''
    contours, hierarchy = cv2.findContours(mask.copy(), mode=cv2.RETR_CCOMP,
                                           method=cv2.CHAIN_APPROX_NONE)
    max_area = 0
    max_cnt = None
    for cnt_idx, cnt in enumerate(contours):
        ...
        if min_area > 0 and cv2.contourArea(cnt) < min_area:
            continue
        if min_convex_rate > 0 and not is_roughly_convex(cnt,
                                                         threshold=min_convex_rate):
            continue
...
def get_closest_contour(contours, hierarchy, ref_loc, min_span=0, min_length=0,
                        hierarchy_req=None):
    '''
    Get a contour closest to the reference point @ref_loc, with minimal size
    (span) of @min_span
    @hierarchy_req specifies whether we are interested in only the inner
    contours, outer contours, or both
    '''
    min_dist = 10000
    closest_cnt = None
    for cnt_idx, cnt in enumerate(contours):
...
def get_closest_blob(mask, ref_loc, min_span=0, min_length=0,
                     hierarchy_req=None):
    mask_closest = np.zeros(mask.shape, dtype=np.uint8)
    contours, hierarchy = cv2.findContours(mask, mode=cv2.RETR_CCOMP,
                                           method=cv2.CHAIN_APPROX_NONE)
    closest_cnt = get_closest_contour(contours, hierarchy, ref_loc, min_span,
                                      min_length, hierarchy_req)
...
def get_small_blobs(mask, max_peri=None, max_area=None, max_span=None):
    ...
    Return the mask with only small blobs, as well as the number of blobs
    remained
    '''
...
def calc_cumsum(input_array):
    ...
    The result is slightly different from numpy's cumsum, so that the sum of
    block
    with row range [i1, i2] and column range [j1, j2] can be expressed as
    cumsum[i2 + 1, j2 + 1] + cumsum[i1, j1] - cumsum[i1, j2 + 1] - cumsum[i2
    + 1, j1]
    '''
...
def display_image(display_name, img, wait_time=-1, is_resize=True,
                  resize_method="max", resize_max=-1, resize_scale=1,
                  save_image=False):
    '''
    Display image at appropriate size. There are two ways to specify the size:
    1. If resize_max is greater than zero, the longer edge (either width or
    height) of the image is set to this value
    ...
    if is_resize:
        ...
        if resize_max > 0:
            if height > width:
                img_display = cv2.resize(img, (
                    resize_max * width / height, resize_max),
                                         interpolation=cv2.INTER_NEAREST)
            else:
                img_display = cv2.resize(img, (
                    resize_max, resize_max * height / width),
                                         interpolation=cv2.INTER_NEAREST)
        elif resize_scale > 0:
            img_display = cv2.resize(img, (
...
def check_and_display(display_name, img, display_list, wait_time=-1,
                      is_resize=True, resize_method="max", resize_max=-1,
                      resize_scale=1, save_image=False):
    if display_name in display_list:
        display_image(display_name, img, wait_time, is_resize, resize_method,
                      resize_max, resize_scale, save_image)


def display_mask(display_name, img, mask, color=(0, 255, 255), wait_time=-1,
                 is_resize=True, resize_method="max", resize_max=-1,
                 resize_scale=1, save_image=False):
    img_display = img.copy()
    img_display[mask > 0, :] = color
    display_image(display_name, img_display, wait_time, is_resize,
                  resize_method, resize_max, resize_scale, save_image)


def check_and_display_mask(display_name, img, mask, display_list,
                           color=(0, 255, 255), wait_time=-1, is_resize=True,
                           resize_method="max", resize_max=-1, resize_scale=1,
                           save_image=False):
    if display_name in display_list:
        display_mask(display_name, img, mask, color, wait_time, is_resize,
                     resize_method, resize_max, resize_scale, save_image)


def plot_bar(bar_data, name='unknown', h=400, w=400, wait_time=-1,
             is_resize=True, resize_method="max", resize_max=-1, resize_scale=1,
             save_image=False):
    ...
    for i, bar_h in enumerate(bar_data):
        cv2.rectangle(plot, (int((i + 1 - 0.3) / (n_items + 1) * w), h), (
            int((i + 1 - 0.3) / (n_items + 1) * w), h - int(bar_h / y_max * h)),
                      [255, 0, 0], -1)
    cv2.putText(plot, "max = %f" % np.max(bar_data), (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 1, [0, 0, 255])
    display_image(name, plot, wait_time, is_resize, resize_method, resize_max,
                  resize_scale, save_image)
...
def vis_detections(img, dets, labels, thresh=0.5):
    ...
    if len(inds) > 0:
        for i in inds:
            ...
            cv2.rectangle(img_detections, (int(bbox[0]), int(bbox[1])),
                          (int(bbox[2]), int(bbox[3])), (0, 0, 255), 8)
            cv2.putText(img_detections, text, (int(bbox[0]), int(bbox[1])),
...
def get_border(img, border_width=5):
    ...
    border[border_width: border.shape[0] - border_width,
    border_width: border.shape[1] - border_width] = False
    return border
...
def is_roughly_square(cnt, th_diff=0.7, th_area=0.6):
    ...
    return (float(diff_p.min()) / diff_p.max() > th_diff) and (
            float(cv2.contourArea(cnt)) / diff_p[0] / diff_p[1] > th_area)
...
def get_corner_pts(bw, perimeter=None, center=None, method='line',
                   is_debug=False, sanity_checks=None):
    '''
    ...
    points
    The function returns @corners as float numbers: [[ul_x, ul_y], [ur_x,
    ur_y], [bl_x, bl_y], [br_x, br_y]]
...
#        i_start += 1
#    i_end_cmp_val = sum_1[bi.shape[0] - 1 - int(round(config.BRICK_HEIGHT /
#    4.0 * 2))] * 0.6
...
#        j_start += 1
#    j_end_cmp_val = sum_0[bi.shape[1] - 1 - int(round(config.BRICK_WIDTH /
#    4.0 * 2))] * 0.6
#    while sum_0[j_end] < j_end_cmp_val:
#        j_end -= 1
#
#    #print (bi.shape, i_start, i_end, j_start, j_end)
#    return img[i_start : i_end + 1, j_start : j_end + 1, :], (i_start,
#    i_end, j_start, j_end)
...
def normalize_color(img, mask_info=None, mask_apply=None, method='hist',
    ...
    if method == 'hist':  # doesn't work well for over-exposed images
        ...
        mask_over_exposed = np.bitwise_and(super_bitwise_or((
...
def color_dist(img, color_space, hsv=None, BGR_ref=[255, 255, 255],
               HSV_ref=[0, 255, 255], useV=False):
    if color_space == 'BGR':
        ...
        if useV:
            v = hsv[:, :, 2]
            dist = np.absolute(np.cos(
                (h.astype(np.int) - HSV_ref[0]) / 180 * np.pi)) * s * np.sqrt(
                (255 - np.abs(v - HSV_ref[2])) / 255)
        else:
            dist = np.absolute(
                np.cos((h.astype(np.int) - HSV_ref[0]) / 180 * np.pi)) * s
        dist = 255 - dist
...
def checkBlurByGradient(img, gradientPatchNBox=5, gradientPatchWidth=25,
                        gradientPatchHeight=25, threshold=500):
    bw = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    ...
    for i in range(gradientPatchNBox):
        for j in range(gradientPatchNBox):
            ...
            bw_window = bw[top: top + gradientPatchHeight,
                        left: left + gradientPatchWidth]
            gradients = np.absolute(
                cv2.Sobel(bw_window, cv2.CV_64F, 1, 1, ksize=5))
...
########################## OBJECT DETECTION ###################################
### http://www.pyimagesearch.com/2015/02/16/faster-non-maximum-suppression
# -python/
...
def object_detection(cv_img, labels, window_method="region",
                     recognition_method="cnn", net=None, transformer=None):
    import dlib

    ## selective search to find candidate bounding boxes
    sk_img = cv_img2sk_img(cv_img)
    rects = []  # locations of the candidates
    dlib.find_candidate_object_locations(sk_img, rects,
                                         min_size=cv_img.shape[0] *
                                                  cv_img.shape[1] / 100)
    ...
    for rect in rects:
        ...
        if aspect_ratio > 0.6 and aspect_ratio < 1.6 \
                and y1 != 0 and y2 != sk_img.shape[0] - 1 and x1 != 0 and x2 \
                != \
                sk_img.shape[1] - 1:
    ...
    data_layer.reshape(len(rect_candidates), data_layer.channels,
                       data_layer.height, data_layer.width)
    for idx, rect in enumerate(rect_candidates):
        x1, y1, x2, y2 = rect
        img_rect = cv_img[y1: y2 + 1, x1: x2 + 1, :]
        net.blobs['data'].data[idx, ...] = transformer.preprocess('data',
                                                                  img_rect)
...
def object_recognition(cv_img, recognition_method="cnn", svm_classifiers=[],
                       net=None, transformer=None, hog_svm=None,
                       hog_descriptor=None):
    if recognition_method == "cnn":
        ## set up input later of net
        data_layer = net.blobs['data']
        data_layer.reshape(1, data_layer.channels, data_layer.height,
                           data_layer.width)
