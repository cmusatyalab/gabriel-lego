...
def detect_color(img_hsv, color, on_surface = False):
    ...
    if color == "black":
        ...
        mask3_1 = color_inrange(None, 'HSV', hsv = img_hsv[0], V_U = 80)
        mask3_2 = color_inrange(None, 'HSV', hsv = img_hsv[1], S_U = 25)
        mask3 = cv2.bitwise_and(mask3_1, mask3_2)
        mask = zc.super_bitwise_or((mask1, mask2, mask3))
    elif color == "white":
        mask = color_inrange(None, 'HSV', hsv = img_hsv, S_U = 60, V_L = 180)
    else:
...
def _find_lego(img, stretch_ratio, display_list):
    ...
    mask_green = zc.expand(mask_green, 3)
    mask_yellow = zc.expand(mask_yellow, 3)
    mask_red = zc.expand(mask_red, 3)
    mask_blue = zc.expand(mask_blue, 3)
    ...
    rtn_msg = {'status' : 'success'}
    return (rtn_msg, (img_lego, img_lego_full, img_board, perspective_mtx))
...
def _reconstruct_lego(img_lego, img_board, rotation_mtx, display_list):
    ...
    img_lego_n0, borders = _lego_outof_board(mask_lego, img_board, rotation_mtx, borders)
    img_lego_n1, borders = _lego_outof_board(mask_lego, img_board, rotation_mtx, borders)
    img_lego_n2, borders = _lego_outof_board(mask_lego, img_board, rotation_mtx, borders)
    img_lego_n3, borders = _lego_outof_board(mask_lego, img_board, rotation_mtx, borders)
    img_lego_n4, borders = _lego_outof_board(mask_lego, img_board, rotation_mtx, borders)
    img_lego_n5, borders = _lego_outof_board(mask_lego, img_board, rotation_mtx, borders)
    img_lego_n6, borders = _lego_outof_board(mask_lego, img_board, rotation_mtx, borders)
    ...
    mask_green_all, mask_green_on = mask_green
    mask_red_all, mask_red_on = mask_red
    mask_yellow_all, mask_yellow_on = mask_yellow
    mask_blue_all, mask_blue_on = mask_blue
    ...
    mask_black = detect_color((hsv_lego, hsv_lego), 'black')
...
def process(img, stretch_ratio, display_list):
    t_start = time.time()
    img = cv2.resize(img, (config.IMAGE_WIDTH / 2, config.IMAGE_HEIGHT / 2), interpolation = cv2.INTER_AREA)
    rtn_msg, objects = _find_lego(img, stretch_ratio, display_list)
    if objects is not None:
        img_lego, img_lego_full, img_board, perspective_mtx = objects
    print rtn_msg
    if rtn_msg['status'] == 'success':
        ...
        print rtn_msg
    if rtn_msg['status'] == 'success':
        rtn_msg, bitmap = _reconstruct_lego(img_lego, img_board, rotation_mtx, display_list)
        print rtn_msg
